/** Benchmark file for SVD test.
 * To be included in main file to run BenchMark for SVD test.
 */
...
void testAccuracy(const std::vector<Eigen::Matrix<T, 2, 2> >& AA,
    const std::vector<Eigen::Matrix<T, 2, 2> >& UU,
    const std::vector<Eigen::Matrix<T, 2, 1> >& SS,
    const std::vector<Eigen::Matrix<T, 2, 2> >& VV)
{
    ...
    for (size_t i = 0; i < AA.size(); i++) {
        Eigen::Matrix<T, 2, 2> M = AA[i];
        Eigen::Matrix<T, 2, 1> S = SS[i];
        Eigen::Matrix<T, 2, 2> U = UU[i];
        Eigen::Matrix<T, 2, 2> V = VV[i];
        T error;
        error = (U * U.transpose() - Eigen::Matrix<T, 2, 2>::Identity()).array().abs().maxCoeff();
        max_UUt_error = (error > max_UUt_error) ? error : max_UUt_error;
        ave_UUt_error += fabs(error);
        error = (V * V.transpose() - Eigen::Matrix<T, 2, 2>::Identity()).array().abs().maxCoeff();
...
template <class T>
void runImplicitQRSVD(const int repeat, const std::vector<Eigen::Matrix<T, 2, 2> >& tests, const bool accuracy_test)
{
    using namespace JIXIE;
    std::vector<Eigen::Matrix<T, 2, 2> > UU, VV;
    std::vector<Eigen::Matrix<T, 2, 1> > SS;
    JIXIE::Timer timer;
    timer.start();
    double total_time = 0;
...
template <class T>
void addRandomCases(std::vector<Eigen::Matrix<T, 2, 2> >& tests, const T random_range, const int N)
{
    using namespace JIXIE;
...
template <class T>
void addIntegerCases(std::vector<Eigen::Matrix<T, 2, 2> >& tests, const int int_range)
{
    using namespace JIXIE;
    int old_count = tests.size();
    ...
    Eigen::Matrix<T, 2, 2> Z;
    Z.fill(-int_range);
    typename Eigen::Matrix<T, 2, 2>::Index i = 0;
    tests.push_back(Z);
    while (i < Eigen::Matrix<T, 2, 2>::SizeAtCompileTime) {
        if (Z(i) < int_range) {
            Z(i)++;
            tests.push_back(Z);
            i = 0;
...
template <class T>
void addPerturbationFromIdentityCases(std::vector<Eigen::Matrix<T, 2, 2> >& tests, const int num_perturbations, const T perturb)
{
    using namespace JIXIE;
    int old_count = tests.size();
    std::vector<Eigen::Matrix<T, 2, 2> > tests_tmp;
    Eigen::Matrix<T, 2, 2> Z = Eigen::Matrix<T, 2, 2>::Identity();
...
template <class T>
void addPerturbationCases(std::vector<Eigen::Matrix<T, 2, 2> >& tests, const int int_range, const int num_perturbations, const T perturb)
{
    using namespace JIXIE;
    int old_count = tests.size();
    std::vector<Eigen::Matrix<T, 2, 2> > tests_tmp;
...
void runBenchmark()
{
    ...
    for (int test_number = 1; test_number <= 10; test_number++) {
        ...
        if (test_float) {
            std::vector<Eigen::Matrix<float, 2, 2> > tests;
            if (test_integer)
        ...
        std::cout << std::setprecision(10) << "\n--- double test ---\n" << std::endl;
        if (test_double) {
            std::vector<Eigen::Matrix<double, 2, 2> > tests;
            if (test_integer)
                addIntegerCases(tests, integer_range);
            if (test_perturbation)
